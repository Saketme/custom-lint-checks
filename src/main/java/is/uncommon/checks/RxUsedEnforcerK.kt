  ...
  override fun createUastHandler(context: JavaContext): UElementHandler? {
    ...
    return object : UElementHandler() {
      override fun visitCallExpression(node: UCallExpression) {
        ...
        val valid = RX_PRIMITIVE_CANONICAL_NAMES.contains(returnType.canonicalText) &&
            node.uastParent !is JavaUCompositeQualifiedExpression &&
            node.uastParent !is JavaULocalVariable &&
            node.uastParent !is JavaUReturnExpression

        println(
            "reporting from visitCallExpression => " + node + " => " + node.uastParent!!.javaClass.canonicalName + " => " + returnType
                .canonicalText + " => valid=" + valid)
        if (valid) {
